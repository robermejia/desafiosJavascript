...
const $contenedorDeCards = document.getElementById('contenedor-productos');
const $contenedorDeCarrito = document.getElementById('contenedorCarrito');
const $contadorCarrito1 = document.getElementById('contadorCarrito1');
...
const mostrarEnCarrito = (platoAgregar) => {
    ...
    div.innerHTML= `<li class="list-group-item d-flex justify-content-between alto-input">                        
                        <h6 class=" in-nombre" >${platoAgregar.nombre}</h6>                        
                        <p class="in-cant" id="cant${platoAgregar.id}">Cant.:${platoAgregar.cantidad}</p>
                        <i class="bi bi-trash in-icono" id="eliminar${platoAgregar.id}"></i>
                        <span class="text-muted in-precio">S./${platoAgregar.precio}</span>
                    </li>` 
    ...
    btnEliminar.addEventListener('click',() => {
        if(platoAgregar.cantidad == 1) {
            ...
            document.getElementById(`cant${platoAgregar.id}`).innerHTML = `<p id="cant${platoAgregar.id}">Cant.:${platoAgregar.cantidad}</p>`
            actualizarCarrito()
        }
    })    
}
...
const agregarAlCarrito = (id) =>{
    ...
    if(existe){
        ...
        document.getElementById(`cant${existe.id}`).innerHTML = `<p id="cant${existe.id}">Cant.:${existe.cantidad}</p>`
        actualizarCarrito()
    }else{
